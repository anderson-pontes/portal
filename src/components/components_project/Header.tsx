...
import { Home, Menu, CircleAlert, FileText } from "lucide-react";
...
const Pages = [
  ...
  { id: 2, name: "Avisos", icon: CircleAlert, path: "/avisos" },
  { id: 3, name: "Manuais", icon: FileText, path: "/manuais" },
];
export function Header() {
  ...
  return (
    <div
      className={`glass-header border-b border-white/20 ${isMenuOpen ? "mb-14" : ""}`}
    >
      <div className="flex h-16 items-center gap-6 justify-start w-screen">
        <div className="flex items-center gap-6 pl-6">
          ...
          <Separator
            ...
            className="h-8 hidden lg:block bg-gradient-to-b from-transparent via-slate-300/50 to-transparent"
          />
        </div>
        ...
        {/* Menu Navigation */}
        <nav
          className={`flex-col text-slate-700 lg:flex-row lg:flex items-start lg:items-center space-x-0 lg:space-x-6 lg:space-y-0 space-y-4 absolute lg:relative top-16 lg:top-0 left-0 w-full lg:w-auto p-4 lg:p-0 z-10 lg:z-auto transition-all duration-300 ease-in-out ${
            isMenuOpen
              ? "flex glass-header shadow-lg border-t border-white/20"
              : "hidden"
          }`}
        >
          {Pages.map((page) => (
            <NavLink key={page.id} to={page.path} onClick={closeMenu}>
