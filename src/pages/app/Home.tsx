...
export function Home() {
  ...
  const flashcardsUteis: Flashcard[] = [
    { title: "ATTUS PGE", description: "Sistema de apoio à gestão de processos jurídicos e execução fiscal", image: imgAttusPge, externalLink: "https://pgepa.attus.ai/" },
    { title: "SITE PGE", description: "Portal institucional da Procuradoria-Geral do Estado do Pará", image: imgBrasaoPge, externalLink: "https://www.pge.pa.gov.br/" },
    { title: "ZIMBRA", description: "Plataforma de e-mail e colaboração corporativa da Procuradoria-Geral do Estado do Pará", image: imgZimbra, externalLink: "https://mail.pa.gov.br/" },
    { title: "PJE TJPA", description: "Sistema de tramitação de processos judiciais eletrônicos", image: imgPje, externalLink: "https://www.tjpa.jus.br/PortalExterno/institucional/Portal-PJE/942-Apresentacao.xhtml" },
    { title: "GOVERNO DIGITAL", description: "Sistema de centralização dos sistemas do Governo do Pará", image: imgProdepa, externalLink: "https://sistemas.pa.gov.br/" }
  ];
  ...
  const CardContent = ({ card }: { card: Flashcard }) => (
    <>
      ...
      <div className="relative z-10 w-16 h-16 shrink-0 bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 rounded-xl p-3 flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform duration-300 group-hover:rotate-3">
        <img src={card.image} alt={card.title} className="w-full h-full object-contain animate-float" />
      </div>
  ...
  const renderGrid = (cards: Flashcard[], gridClasses: string) => (
    <div className={`grid gap-x-6 gap-y-8 w-full ${gridClasses}`}>
      {cards.map((card, index) => {
        ...
        return (
          <div key={index} className="animate-slideIn w-full relative" style={{ animationDelay: `${index * 0.05}s` }}>
            {card.externalLink ? (
              <a href={card.externalLink} target="_blank" rel="noopener noreferrer" className={cardClasses}>
            ...
            <Popover>
              ...
              <PopoverContent align="end" sideOffset={8} className="sm:hidden w-64 text-sm">
                {card.description}
              </PopoverContent>
            </Popover>
          </div>
    ...
    <>
      ...
      <div className="relative w-full flex flex-col pt-6 px-4 min-h-screen pb-24">
        <div className="max-w-7xl w-full mx-auto flex flex-col h-full">
          ...
          {renderGrid(flashcards, "grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4")}

          {/* Separador Moderno (Badge Divisor) */}
          <div className="w-full mt-10 mb-10 animate-slideIn" style={{ animationDelay: "0.6s" }}>
          ...
          {renderGrid(flashcardsUteis, "grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4")}
